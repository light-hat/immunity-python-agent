...
from ._generated import Mnemonics, RegisterMasks, Registers
...
class _DInst(Structure):
    _fields_ = [
        ...
        (
            "meta",
            c_uint16,
        ),  # meta flags - instruction set class, etc. See C headers again...
        (
            "modifiedFlagsMask",
            c_uint16,
        ),  # CPU modified (output) flags by instruction only set with DF_FILL_EFLAGS
        (
            "testedFlagsMask",
            c_uint16,
        ),  # CPU tested (input) flags by instruction only set with DF_FILL_EFLAGS
        (
            "undefinedFlagsMask",
            c_uint16,
        ),  # CPU undefined flags by instruction only set with DF_FILL_EFLAGS
    ]
...
OPERAND_FAR_MEMORY = (
    "FarMemory"  # like absolute but with selector/segment specified too
)
...
class FlowControl:
    ...
    (CALL, RET, SYS, UNC_BRANCH, CND_BRANCH, INT, CMOV, HLT) = range(1, 9)
...
class Instruction(object):
    ...
    def _extractOperand(self, di, operand):
        ...
        if operand.type == O_IMM:
            ...
            return Operand(
                OPERAND_IMMEDIATE,
                _unsignedToSigned(di.imm.addr) + self.address + self.size,
                operand.size,
            )
        elif operand.type == O_PTR:
            return Operand(
                OPERAND_FAR_MEMORY, di.imm.ptr.seg, di.imm.ptr.off, operand.size
            )
        else:
...
def DecomposeGenerator(codeOffset, code, dt, features=0):
    ...
    while codeLen > 0:
        ...
        codeInfo = _CodeInfo(
            ...
            _OffsetType(0),
            _OffsetType(0),
            cast(p_code, c_char_p),
            codeLen,
            dt,
            features,
        )
        status = internal_decompose(
            byref(codeInfo),
            byref(result),
            MAX_INSTRUCTIONS,
            byref(usedInstructionsCount),
        ...
        for index in range(used):
            ...
            yield Instruction(
                di,
                code[di.addr - startCodeOffset : di.addr - startCodeOffset + di.size],
                dt,
            )
