...
class InstructionInfo:
    ...
    def __init__(self, classType, OL, pos, isModRMIncluded, mnemonics, operands, flags):
        ...
        if (
            len(
                list(
                    filter(
                        lambda x: x
                        in [OperandType.VXMM, OperandType.VYMM, OperandType.VYXMM],
                        self.operands,
                    )
                )
            )
        ...
        if self.flags & InstFlag.PRE_VEX:
            ...
            self.VEXtag = "V"
            if self.flags & (InstFlag.MODRR_BASED | InstFlag.VEX_V_UNUSED) == (
                InstFlag.MODRR_BASED | InstFlag.VEX_V_UNUSED
            ):
...
class InstructionsDB:
    ...
    def HandleMandatoryPrefix(self, type, o, pos, ii, tag):
        ...
        if (
            isinstance(o.list[pos[0]], InstructionsTable)
            and o.list[pos[0]].type == NodeType.LIST_PREFIXED
        ):
    ...
    def CreateSet(self, type, o, pos, ii, tag="", level=0):
        ...
        self.CreateSet(
            OpcodeLength.NextOL[type], o.list[pos[0]], pos[1:], ii, tag, level + 1
        )
    ...
    def SetInstruction(self, *args):
        ...
        if (args[4] & InstFlag.EXPORTED) != 0:
            ii = InstructionInfo(
                args[0], OpcodeLength.OL_1, [0], False, args[2], args[3], args[4]
            )
        ...
        if last[:2] == "//":  # Divided Instruction
            ...
            try:
                OL = {
                    1: OpcodeLength.OL_13,
                    2: OpcodeLength.OL_23,
                    3: OpcodeLength.OL_33,
                }[len(opcode)]
            except KeyError:
        ...
        ii = InstructionInfo(
            args[0], OL, pos, isModRMIncluded, args[2], args[3], args[4]
        )
