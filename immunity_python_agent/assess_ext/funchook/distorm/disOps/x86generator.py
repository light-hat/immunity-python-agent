...
def CheckOTCollisions(ii):
    ...
    for i in types:
        if i != O_REG and types.count(i) > 1:
            print(
                "**WARNING: Operand type collision for instruction: " + ii.mnemonics[0],
                ii.tag,
            )
            break
...
def UpdatePrivilegedInstruction(opcodeIds, ii):
    ...
    def IsPrivilegedMov(ii):
        ...
        return (ii.mnemonics[0] == "MOV") and (
            (OperandType.CREG in ii.operands) or (OperandType.DREG in ii.operands)
        )
...
def UpdateInstructionAffectedFlags(ii):
    ...
    for i in ["SET", "CMOV", "FCMOV"]:
        if ii.mnemonics[0].find(i) == 0:
            SetInstructionAffectedFlags(
                ii, GetTestedFlagsForCondition(ii.mnemonics[0][len(i) :])
            )
            return
    # See if it's a Jcc instruction.
    if ii.mnemonics[0][:1] == "J" and ii.mnemonics[0][:2] not in [
        "JM",
        "JC",
        "JE",
        "JR",
...
def FormatInstruction(ii, mnemonicsIds):
    ...
    mnems = TranslateMnemonics(
        [None, ii.classType][
            (ii.flags & InstFlag.PSEUDO_OPCODE) == InstFlag.PSEUDO_OPCODE
        ],
        ii.mnemonics,
        mnemonicsIds,
    ) + [0, 0]
    ...
    if isExtended:
        ...
        op3 = [OperandType.NONE, ops[2]][
            (ii.flags & InstFlag.USE_OP3) == InstFlag.USE_OP3
        ]
        op4 = [OperandType.NONE, ops[3]][
            (ii.flags & InstFlag.USE_OP4) == InstFlag.USE_OP4
        ]
        if flagsEx >= 256:  # Assert the size of flagsEx is enough to holds this value.
            raise Exception(
                "FlagsEx exceeded its 8 bits. Change flagsEx of _InstInfoEx to be uint16!"
            )
    ...
    if flagsIndex >= 256:
        raise Exception(
            "FlagsIndex exceeded its 8 bits. Change flags of _InstInfo to be uint16!"
        )
    ...
    sharedInfo = (
        ...
        ii.testedFlags,
        ii.undefinedFlags,
        (ii.classType << 8) | ii.flowControl | privileged,
    )
    ...
    if sharedInfoIndex >= 2**16:
        raise Exception(
            "SharedInfoIndex exceeded its 16 bits. Change type of sharedInfoIndex in _InstInfo!"
        )
    ...
    if ii.flags & InstFlag.EXTENDED:
        fields = (
            "{%s}" % fields
        )  # Extra parentheses for sub structure in case it's InstInfoEx.
    # "Structure-Name" = II_Bytes-Code {Fields + Optional-Fields}.
    return (
        "\t/*II%s*/ {%s%s}" % (ii.tag, fields, optFields),
        (ii.flags & InstFlag.EXTENDED) != 0,
    )
        ...
        for i in x86db.GenBlock(x):
            if isinstance(i, x86db.InstructionInfo):
                ...
                if isExtended:
                    ...
                    InstructionsTree.append(
                        (NodeType.INFOEX << typeShift | index, i.tag)
                    )
                else:
                ...
                nextTableIndex += (
                    i.size
                )  # This assumes we walk on the instructions tables in BFS order!
            else:
    ...
    s = [
        "\n".join(
            [
                "_InstInfo II_%s =%s;"
                % (
                    i.mnemonics[0] if i.mnemonics[0][0] != "_" else i.mnemonics[0][1:],
                    FormatInstruction(i, mnemonicsIds)[0],
                )
                for i in db.getExportedInstructions()
            ]
        ),
        ...
        % (
            ...
            ",\n".join(
                [
                    "0x%x" % i[1]
                    for i in sorted(zip(flagsDict.values(), flagsDict.keys()))
                ]
            ),
        ),
        ...
        % (
            ...
            ",\n".join(
                [
                    "/* %x - %s */  %s"
                    % (i[0], i[1][1], "0" if i[1][0] == 0 else "0x%x" % i[1][0])
                    for i in enumerate(InstructionsTree)
                ]
            ),
        ),
        ...
        % (
            ...
            ",\n".join(
                [
                    "{%s}" % str(i[1])[1:-1]
                    for i in sorted(zip(sharedInfoDict.values(), sharedInfoDict.keys()))
                ]
            ),
        ),
