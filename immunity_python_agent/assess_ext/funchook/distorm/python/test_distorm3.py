...
import distorm3
from distorm3._generated import Mnemonics, Registers
...
def Assemble(text, mode):
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".asm", prefix="distorm3-test-", mode="wb+", delete=False
    ) as asm_file:
...
class TestMode64(unittest.TestCase):
    ...
    def test_moffs8(self):
        ...
        I64("mov [qword 0xaaaabbbbccccdddd], al").check_abs_disp(
            0, 0xAAAABBBBCCCCDDDD, 64, 8
        )

    def test_moff_full(self):
        I64("mov [dword 0xaaaabbbb], rax").check_abs_disp(0, 0xFFFFFFFFAAAABBBB, 32, 64)
        I64("mov rax, [qword 0xaaaabbbbccccdddd]").check_abs_disp(
            1, 0xAAAABBBBCCCCDDDD, 64, 64
        )
...
class TestInstTable(unittest.TestCase):
    ...
    def test_vex(self):
        I32("vaddpd ymm1, ymm2, ymm2").check_mnemonic(
            "VADDPD"
        )  # pre encoding: 66, 0f, 58
        I32("vaddps ymm1, ymm2, ymm2").check_mnemonic("VADDPS")  # pre encoding: 0f, 58
        I32("vaddsd xmm1, xmm2, qword [eax]").check_mnemonic(
            "VADDSD"
        )  # pre encoding: f2, 0f, 58
        I32("vaddss xmm1, xmm2, dword [eax]").check_mnemonic(
            "VADDSS"
        )  # pre encoding: f3, 0f, 58
        I32("vmovsd xmm1, xmm2, xmm3").check_mnemonic(
            "VMOVSD"
        )  # pre encoding: f2, 0f, 10
        I32("vmovsd xmm1, qword [eax]").check_mnemonic(
            "VMOVSD"
        )  # pre encoding: f2 0f 10 - but VEX.vvvv is not encoded!
        # Since in a VEX prefix you can encode the virtual prefix, we got three ways to get to 0xf 0x38
        # So see that both work well.
        IB32("c4e279dbc2").check_mnemonic(
            "VAESIMC"
        )  # pre encoding: 66, 0f, 38, db, virtual prefix is 0f 38
        IB32("c4e17938dbc2").check_mnemonic("VAESIMC")  # the virtual prefix is only 0f
        IB32("c5f938dbc2").check_mnemonic(
            "VAESIMC"
        )  # the virtual prefix is only 0f, but short VEX
        # Same test as earlier, but for 0xf 0x3a, though this instruction doesn't have a short form.
        IB32("c4e3710dc255").check_mnemonic(
            "VBLENDPD"
        )  # pre encoding: 66, 0f, 3a, 0d, virtual prefix is 0f 3a
        IB32("c4e1713a0dc255").check_mnemonic(
            "VBLENDPD"
        )  # pre encoding: 66, 0f, 3a, 0d, virtual prefix is 0f
    ...
    def test_vex_special(self):
        ...
        IB32("c4e2791800").check_mnemonic(
            "VBROADCASTSS"
        )  # just to make sure this instruction is fine.
...
class TestMisc(unittest.TestCase):
    ...
    def test_branch_hints(self):
        ...
        self.assertFalse(
            "FLAG_HINT_NOT_TAKEN" not in I32("db 0x2e\n jp 0x55").inst.flags
        )
        self.assertFalse(
...
class TestMisc2(unittest.TestCase):
    ...
    def test_longest_mnemonic(self):
        ...
        self.assertEqual(
            I32("VAESKEYGENASSIST xmm1, xmm2, 7").inst.mnemonic, "VAESKEYGENASSIST"
        )
...
class TestPrefixes(unittest.TestCase):
    ...
    def test_stos(self):
        ...
        self.assertEqual(
            str(IB16("2eab").inst), "STOSW"
        )  # 1st op cannot be prefixed by segment!
        ...
        self.assertEqual(
            str(IB64("2eab").inst), "STOSD"
        )  # 1st op cannot be prefixed by segment!
        self.assertEqual(
            str(IB64("2e48ab").inst), "STOSQ"
        )  # 1st op cannot be prefixed by segment!
    ...
    def test_scas(self):
        ...
        self.assertEqual(
            str(IB64("2eaf").inst), "SCASD"
        )  # 1st op cannot be prefixed by segment!
        self.assertEqual(
            str(IB64("2e48af").inst), "SCASQ"
        )  # 1st op cannot be prefixed by segment!
    ...
    def test_movs(self):
        ...
        self.assertEqual(
            str(IB16("f3656766a5").inst), "REP MOVS DWORD [ES:EDI], [GS:ESI]"
        )
        self.assertEqual(
            str(IB16("6667f365a5").inst), "REP MOVS DWORD [ES:EDI], [GS:ESI]"
        )
        self.assertEqual(str(IB16("6667f3a4").inst), "REP MOVS BYTE [ES:EDI], [DS:ESI]")
        self.assertEqual(
            str(IB16("6667f2a4").inst), "REPNZ MOVS BYTE [ES:EDI], [DS:ESI]"
        )
        # 32 bits
        ...
        self.assertEqual(
            str(IB64("4fa5").inst), "MOVSQ"
        )  # Set all REX bits, still MOVSQ.
        ...
        self.assertEqual(
            str(IB64("6667f36548a5").inst), "REP MOVS QWORD [EDI], [GS:ESI]"
        )
    ...
    def test_cmps(self):
        ...
        self.assertEqual(
            str(IB16("f36567a7").inst), "REPZ CMPS WORD [GS:ESI], [ES:EDI]"
        )
        self.assertEqual(
            str(IB16("f3656766a7").inst), "REPZ CMPS DWORD [GS:ESI], [ES:EDI]"
        )
        self.assertEqual(
            str(IB16("6667f365a7").inst), "REPZ CMPS DWORD [GS:ESI], [ES:EDI]"
        )
        self.assertEqual(
            str(IB16("6667f3a6").inst), "REPZ CMPS BYTE [DS:ESI], [ES:EDI]"
        )
        self.assertEqual(
            str(IB16("6667f2a6").inst), "REPNZ CMPS BYTE [DS:ESI], [ES:EDI]"
        )
        # 32 bits
        ...
        self.assertEqual(
            str(IB32("f3656766a7").inst), "REPZ CMPS WORD [GS:SI], [ES:DI]"
        )
        self.assertEqual(
            str(IB32("6667f365a7").inst), "REPZ CMPS WORD [GS:SI], [ES:DI]"
        )
        ...
        self.assertEqual(
            str(IB64("4fa7").inst), "CMPSQ"
        )  # Set all REX bits, still CMPSQ.
        ...
        self.assertEqual(
            str(IB64("6667f36548a7").inst), "REPZ CMPS QWORD [GS:ESI], [EDI]"
        )
...
class TestFeatures(unittest.TestCase):
    ...
    def test_fc(self):
        pairs = [
            ...
            (
                ...
                distorm3.FlowControl.CND_BRANCH,
            ),
        ]
        for i in pairs:
            for j in i[0]:
    ...
    def test_eflags_on(self):
        ...
        # XOR
        self.assertEqual(
            a[0].modifiedFlags, distorm3.D_SF | distorm3.D_ZF | distorm3.D_PF
        )
    ...
    def test_stop_undecodable(self):
        self.assertEqual(
            len(IB16("909033c0ffff90", distorm3.DF_STOP_ON_UNDECODEABLE).insts), 4
        )
        self.assertEqual(
            len(IB32("909033c090ffff90", distorm3.DF_STOP_ON_UNDECODEABLE).insts), 5
        )
        self.assertEqual(
            len(IB64("909033c09090ffff9090", distorm3.DF_STOP_ON_UNDECODEABLE).insts), 6
        )
...
class TestAPI(unittest.TestCase):
    def direct_decompose(self, code, codeOffset, dt, features, maxInstructions):
        ...
        codeInfo = distorm3._CodeInfo(
            distorm3._OffsetType(codeOffset),
            distorm3._OffsetType(0),
            distorm3._OffsetType(0),
            ctypes.cast(p_code, ctypes.c_char_p),
            codeLen,
            dt,
            features,
        )
        status = distorm3.internal_decompose(
            ctypes.byref(codeInfo),
            ctypes.byref(result),
            maxInstructions,
    ...
    def test_out_buf(self):
        s, count, results = self.direct_decompose(
            b"\x90\x90", 0, distorm3.Decode32Bits, 0, 0
        )
        self.assertEqual(s, distorm3.DECRES_INPUTERR)
        s, count, results = self.direct_decompose(
            b"\x90\x90", 0, distorm3.Decode32Bits, 0, 1
        )
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 1)
        s, count, results = self.direct_decompose(
            b"\x90\x90", 0, distorm3.Decode32Bits, 0, 2
        )
    ...
    def test_0_len(self):
        ...
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        s, count, results = self.direct_decompose(
            b"", 0x1234, distorm3.Decode64Bits, 0, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
    ...
    def test_bad_features(self):
        s, count, results = self.direct_decompose(
            ...
            distorm3.DF_MAXIMUM_ADDR16 | distorm3.DF_MAXIMUM_ADDR32,
            1,
        )
        self.assertEqual(s, distorm3.DECRES_INPUTERR)
    ...
    def test_single_out_buf(self):
        s, count, results = self.direct_decompose(
            b"\x66\x90", 0, distorm3.Decode32Bits, 0, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "NOP")
        s, count, results = self.direct_decompose(
            b"\x66\x67", 0, distorm3.Decode32Bits, 0, 1
        )
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 1)
        self.assertEqual(results[0].imm.byte, 0x66)
        s, count, results = self.direct_decompose(
            b"\x66\x67", 0, distorm3.Decode32Bits, 0, 2
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 2)
        self.assertEqual(results[0].imm.byte, 0x66)
    ...
    def test_drop_skip(self):
        s, count, results = self.direct_decompose(
            b"\x05\x00\x01", 0, distorm3.Decode32Bits, 0, 1
        )  # Skips 05, then returns add [ecx], al
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 1)
        s, count, results = self.direct_decompose(
            b"\x05\x00\x01", 0, distorm3.Decode32Bits, 0, 2
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 2)
        s, count, results = self.direct_decompose(
            b"\xc4\x01", 0, distorm3.Decode32Bits, 0, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "LES")
        s, count, results = self.direct_decompose(
            b"\xc5\xc5", 0, distorm3.Decode32Bits, 0, 2
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 2)
        ...
        s, count, results = self.direct_decompose(
            b"\xc5\xc5\xc5", 0, distorm3.Decode32Bits, 0, 3
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
    ...
    def test_fc(self):
        s, count, results = self.direct_decompose(
            b"\x90", 0, distorm3.Decode32Bits, distorm3.DF_RETURN_FC_ONLY, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 0)
        s, count, results = self.direct_decompose(
            b"\x90\x75\x00", 0, distorm3.Decode32Bits, distorm3.DF_RETURN_FC_ONLY, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
    ...
    def test_single_step(self):
        s, count, results = self.direct_decompose(
            b"\x33\xff\xc0", 0, distorm3.Decode32Bits, distorm3.DF_SINGLE_BYTE_STEP, 1
        )
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 1)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "XOR")
        s, count, results = self.direct_decompose(
            b"\x33\xff\xc0", 0, distorm3.Decode32Bits, distorm3.DF_SINGLE_BYTE_STEP, 2
        )
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 2)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "XOR")
        ...
        s, count, results = self.direct_decompose(
            b"\x33\xff\xc0", 0, distorm3.Decode32Bits, distorm3.DF_SINGLE_BYTE_STEP, 3
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 3)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "XOR")
    ...
    def test_fc_and_single_step(self):
        s, count, results = self.direct_decompose(
            ...
            0,
            distorm3.Decode32Bits,
            distorm3.DF_SINGLE_BYTE_STEP | distorm3.DF_RETURN_FC_ONLY,
            1,
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
    ...
    def test_fc_dropped(self):
        s, count, results = self.direct_decompose(
            b"\x33\xc3", 0, distorm3.Decode64Bits, distorm3.DF_RETURN_FC_ONLY, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 0)
        s, count, results = self.direct_decompose(
            b"\x66\x67\x90\xc3", 0, distorm3.Decode64Bits, distorm3.DF_RETURN_FC_ONLY, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
        s, count, results = self.direct_decompose(
            b"\x66\x67", 0, distorm3.Decode64Bits, distorm3.DF_RETURN_FC_ONLY, 1
        )
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 0)
